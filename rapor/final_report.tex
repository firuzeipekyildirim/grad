...
The proposed optimization model is formulated as a Capacitated Vehicle Routing Problem
...
The prioritized penalty structure embedded in the objective function ensures that delayed
express deliveries incur the highest cost, followed by express, delayed normal, and normal
deliveries. This structure directly influences the resulting routes and vehicle assignments,
...
Only one vehicle can be used in attending to each customer to prevent duplication.
...
The vehicle comes into effect as a serves-one-customer vehicle, thus determining that fixed costs are only charged based on the utilization of the vehicles.
...
These constraints ensure route continuity by guaranteeing only one incoming and one outgoing arc to all visited customers.
...
Any given vehicle that starts must start from the depot and return after finishing his duty.
...
These constraints limit the number of customers served by each vehicle to a
predefined operational interval.
...
These constraints ensure that vehicle loads dynamically decrease due to
deliveries and increase due to pickups, while always remaining within capacity limits.
...
These constraints ensure temporal feasibility by accounting for travel times
and service durations along each route.
...
The Miller–Tucker–Zemlin (MTZ) constraints prevent the formation of subtours
disconnected from the depot, ensuring a single continuous route for each vehicle.
...
This situation is often not intentional. The algorithm simply chooses the cheapest route. But the result can still be unfair. Therefore, without corrective mechanisms, the route planning system can increase inequality. For example, rules must be established that require the service to reach every region at a certain minimum level, or a weighting system must be implemented that does not completely overlook disadvantaged areas. Otherwise, while the system may appear efficient, it invisibly makes access to the service more difficult for some people.
\paragraph{Relevant UNSDGs}
\begin{itemize}
    \item \textbf{SDG 10.2 (Primary):} Strengthen the social and economic inclusion of all by 2030~\cite{UN2015}.
    \item \textbf{SDG 10.3 (Secondary):} Reducing inequality in outcomes and ensuring equal opportunity~\cite{UN2015}.
\end{itemize}
